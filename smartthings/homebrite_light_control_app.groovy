/**
 *  RESTful HomeBrite CSRMesh Light Controller
 *
...
definition(
    name: "RESTful CSRMesh Dimmable Light",
    namespace: "restful_csrmesh_light",
    author: "Dan Isla",
    description: "RESTful HomeBrite CSRMesh Light Control",
...
preferences {
    page(name:"selectDevPage", title:"Configuration", content:"selectDevPage")
}
...
def selectDevPage() {
    ...
    sendHubCommand(new physicalgraph.device.HubAction("lan discovery urn:schemas-upnp-org:device:DimmableLight:", physicalgraph.device.Protocol.LAN))

    def devicesForDialog = getDevicesForDialog()

    // Only one page - can install or uninstall from this page
    return dynamicPage(name:"selectDevPage", title:"", nextPage:"", refreshInterval: refreshInterval, install:true, uninstall: true) {
        section("") {
            input "selectDev", "enum", required:false, title:"Select LAN Device \n(${devicesForDialog.size() ?: 0} found)", multiple:true, options:devicesForDialog
        }
    }
}
...
def locationHandler(evt) {
    ...
    log.debug('RESTful CSRMesh Light Control App Received Response: ' + evt.description)
    ...
    if (parsedEvent?.ssdpTerm?.contains("schemas-upnp-org:device:DimmableLight:")) {
        def devices = getDevices()
...
def initialize() {
    ...
    selectDev.each { ssdpUSN ->
        def devices = getDevices()

        // Make the dni the MAC followed by the index from the USN, unless it's the USN ending in :1
        // that device has just the MAC address as its DNI and receives all the notifications
        ...
        if (!d) {
            ...
            d = addChildDevice("restful_csrmesh_light", "RESTful CSRMesh Light Control", dni, devices[ssdpUSN].hub, [
